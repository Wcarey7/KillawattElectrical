...
# On DB init will create "shadow history" tables.
# Inserts rows into corresponding history table on updates and deletes of data.
#
# Adapted from:
...
def _history_mapper(local_mapper):
    ...
    if (
        ...
        # "transaction_type" column stores the action,
        ...
        # "request_id" if multiple models changed
        # in one request, all will have same request_id
        # This column is optional and can be omitted.
        history_table.append_column(
            Column(
                "request_id",
    ...
    if super_history_mapper:
        ...
        if history_table is not None:
            ...
            properties["transaction_type"] = (history_table.c.transaction_type,) + tuple(
                super_history_mapper.attrs.transaction_type.columns
            )
            properties["request_id"] = (history_table.c.request_id,) + tuple(
                super_history_mapper.attrs.request_id.columns
            )
...
def create_version(obj, session, transaction_type, deleted=False):
    ...
    attr["transaction_type"] = transaction_type
    attr["request_id"] = g.request_id
    hist = history_cls()
...
def versioned_session(session):
    ...
    def before_flush(session, flush_context, instances):
        for obj in versioned_objects(session.dirty):
            ...
            create_version(obj, session, transaction_type)
        for obj in versioned_objects(session.deleted):
            transaction_type = "Delete"
            create_version(obj, session, transaction_type, deleted=True)
